...
/// and no change.
pub fn send_max_funds_to_sapling_proposed_transfer(dsf: impl DataStoreFactory, cache: impl TestCache) {
    let mut st = TestBuilder::new()
        .with_data_store_factory(dsf)
        .with_block_cache(cache)
        .with_account_from_sapling_activation(BlockHash([0; 32]))
...
/// Checks that the created transaction has the appropriate notes:  1 Orchard
/// and no change.
pub fn send_max_funds_to_orchard_proposed_transfer(dsf: impl DataStoreFactory, cache: impl TestCache) {
    let mut st = TestBuilder::new()
        .with_data_store_factory(dsf)
        .with_block_cache(cache)
        .with_account_from_sapling_activation(BlockHash([0; 32]))
