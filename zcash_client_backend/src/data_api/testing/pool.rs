...
/// `birthday` to `chain_tip` and any rewind-then-rescan flow can drive a
/// continuous re-scan.
///
/// In the diagram, `X` is the wallet's note commitment, `f` counts
/// non-wallet filler commitments, and `N` is `trailing_filler_blocks`. The
/// shard 1/2 boundary falls inside Block B, whose 30 leaves split 21/9
/// across it.
///
/// ```text
/// blocks:  |<-(faked state)->|<------A------>|<------B------>|<-----N blk----->|
///      birthday-1        birthday        birthday+1      birthday+2        chain_tip
/// leaves:  |<---(2^17-71)--->|<----X+49f---->|<-----30f----->|<------N f------>|
    ...
    <Dsf as DataStoreFactory>::AccountId: std::fmt::Debug,
{
    ...
    let mut st = build_stable_shard_fixture::<T, Dsf>(ds_factory, cache);
...
/// which is `birthday` (Block A's checkpoint). Truncation removes
/// shardtree state strictly above `birthday`, which discards the leaves
/// inserted by Block B (positions 131051..131080). Shard 1's last leaf
/// (position 131071) is among those discarded, so shard 1 reverts from
/// complete to partial.
///
...
/// `witness_anchor_stable` is still set and the note's own transaction is
/// still mined. Recovery requires rescanning Block B (and the trailing
/// filler blocks) so that shard 1's last leaf is reinserted and the shard
/// returns to complete + scan-clean.
pub fn stabilized_note_rewind_un_mines_shard_completion<T, Dsf>(
    ...
    <Dsf as DataStoreFactory>::AccountId: std::fmt::Debug,
{
    let mut st = build_tip_shard_fixture::<T, Dsf>(ds_factory, cache);
    let account = st.test_account().unwrap().clone();
    let birthday_height = st
        .wallet()
        .get_wallet_birthday()
    ...
    // Rewind to a height well below the birthday. Truncation lands at
    // `birthday` (Block A's checkpoint), removing Block B's leaves and
    // reverting shard 1 to partial.
    ...
    // Recover by re-scanning all real blocks from Block A through the chain
    // tip. The block cache is contiguous (no fake-advance gap), so a single
    // `scan_cached_blocks` call drives the full re-scan. After this, shard 1
    // is complete and scan-clean again.
    let real_block_count = 1 + 1 + 5; // Block A + Block B + 5 trailing fillers
    st.scan_cached_blocks(birthday_height, real_block_count as usize);

    assert_eq!(
        st.get_spendable_balance(account.id(), ConfirmationsPolicy::MIN),
        SHARD_1_NOTE_VALUE,
        "after re-scanning Block B and the trailing fillers, shard 1 is complete and the \
