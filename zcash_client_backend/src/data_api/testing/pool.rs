...
/// Tests sending all the spendable funds within the given shielded pool in a
/// single transaction.
///
/// The test:
/// - Adds funds to the wallet in a single note.
...
/// - Checks that the transaction was stored, and that the outputs are decryptable and
///   have the expected details.
pub fn send_max_single_step_proposed_transfer<T: ShieldedPoolTester>(
    dsf: impl DataStoreFactory,
    cache: impl TestCache,
) {
    let mut st = TestBuilder::new()
...
#[cfg(feature = "transparent-inputs")]
pub fn send_multi_step_single_note_max_amount_proposed_transfer<T: ShieldedPoolTester, DSF>(
    ds_factory: DSF,
    cache: impl TestCache,
...
/// checks that the transactions were stored and that the amounts involved are correct
#[cfg(feature = "transparent-inputs")]
pub fn send_multi_step_many_notes_max_amount_proposed_transfer<T: ShieldedPoolTester, DSF>(
    ds_factory: DSF,
    cache: impl TestCache,
    is_reached_gap_limit: impl Fn(&<DSF::DataStore as WalletRead>::Error, DSF::AccountId, u32) -> bool,
) where
...
/// Checks that the transactions were stored and that the amounts involved are correct
#[cfg(feature = "transparent-inputs")]
pub fn send_multi_step_with_marginal_notes_max_amount_proposed_transfer<T: ShieldedPoolTester, DSF>(
    ds_factory: DSF,
    cache: impl TestCache,
    is_reached_gap_limit: impl Fn(&<DSF::DataStore as WalletRead>::Error, DSF::AccountId, u32) -> bool,
) where
    ...
    <DSF as DataStoreFactory>::AccountId: std::fmt::Debug,
{
    ...
    let number_of_marginal_notes = 10u64;
    let number_of_notes = 10u64;
    let note_value = Zatoshis::const_from_u64(100000);
    let marginal_notes_value = (zip317::MARGINAL_FEE * number_of_marginal_notes).expect("product of notes should succeed.");
    let value = (note_value * number_of_notes + marginal_notes_value).unwrap();
    let non_marginal_notes_value = (note_value * number_of_notes).expect("sum of notes should not fail.");
    let run_test = |st: &mut TestState<_, DSF::DataStore, _>, expected_index, prior_balance| {
        // Add funds to the wallet.
        for _ in 0..number_of_notes {
            add_funds(st, note_value);
            add_funds(st, zip317::MARGINAL_FEE);
        }
        ...
        let expected_step1_fee = zip317::MINIMUM_FEE;
        let expected_ephemeral_spend = ( non_marginal_notes_value - expected_step0_fee - expected_step1_fee).unwrap();
        ...
        assert_eq!(steps[0].shielded_inputs().unwrap().notes().len() as u64, number_of_notes);
        assert_eq!(steps[0].balance().fee_required(), expected_step0_fee);
        assert_eq!(steps[1].balance().fee_required(), expected_step1_fee);
        assert_eq!(
            steps[0].balance().proposed_change(),
