...
impl<DbT: InputSource> InputSelector for GreedyInputSelector<DbT> {
        ...
        ChangeT: ChangeStrategy<MetaSource = DbT>,
    {
        ...
        let input_total = spendable_notes.total_value()?;

        let sapling_output_count = {
            let spends_sapling = !spendable_notes.sapling().is_empty();
        ...
        let orchard_action_count = {
            let spends_orchard = !spendable_notes.orchard().is_empty();
            ...
            .convert_if_network(params.network_type())?
        {
            ...
            Address::Unified(addr) => change_strategy.fee_rule().fee_required(
                params,
                target_height,
                [],
                if addr.has_transparent() {
