    ...
    for block in blocks.into_iter() {
        ...
        for tx in block.transactions() {
            ...
            for spend in tx.sapling_spends() {
                wallet_db
                    .mark_sapling_note_spent(tx_ref, spend.nf())
                    .map_err(PutBlocksError::Storage)?;
            }
            #[cfg(feature = "orchard")]
            for spend in tx.orchard_spends() {
                wallet_db
                    .mark_orchard_note_spent(tx_ref, spend.nf())
                    .map_err(PutBlocksError::Storage)?;
            }
    ...
    P: consensus::Parameters,
{
    ...
    {
        wallet_db.mark_sapling_note_spent(tx_ref, spend.nullifier())?;
    }
    #[cfg(feature = "orchard")]
    for action in d_tx
        .tx()
    ...
    {
        wallet_db.mark_orchard_note_spent(tx_ref, action.nullifier())?;
    }
    ...
    {
        wallet_db.mark_transparent_utxo_spent(tx_ref, txin.prevout())?;
    }
