...
pub enum Exposure {
    ...
    Exposed {
        /// The height at which the address was exposed.
        at_height: BlockHeight,
        /// As of the time of the query that produced this exposure metadata, the number of
        /// potentially-exposed addresses at indexes greater than the [`NonHardenedChildIndex`]
        /// corresponding to the address before the gap limit is exhausted for the address's key
        /// scope. `None` if the start of the transparent address gap is at an index greater than
        /// this address's child index; that is, if this address or an address having a greater
        /// child index within the associated key scope has already received funds.
