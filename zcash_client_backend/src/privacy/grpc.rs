...
/// [`PrivateNetwork`].
///
/// The returned client applies the [`PrivateNetwork::timeouts`] of `net` to every request
/// it makes. Note that the request deadline bounds the wait for a response's headers, not
/// the duration of the response itself, so long-running streaming methods such as
/// `GetBlockRange` are not capped by it; a peer that stalls partway through a stream is
/// instead detected by the HTTP/2 keep-alive.
    ...
    N: PrivateNetwork + Clone + 'static,
{
    ...
    let channel = Endpoint::from(endpoint)
        ...
        .http2_keep_alive_interval(timeouts.grpc_keepalive_interval)
        .keep_alive_timeout(timeouts.grpc_keepalive_timeout)
