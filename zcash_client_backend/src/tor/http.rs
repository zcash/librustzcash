...
//! Tor-specific wrappers that surface [`super::Error`]. The [`HttpError`], [`Retry`],
//! [`TimeoutPhase`], and [`cryptex`] items are re-exported from [`crate::privacy::http`]
//! so that existing `tor::http::*` paths continue to resolve.
...
pub use crate::privacy::http::{HttpError, Retry, TimeoutPhase, cryptex};

use super::{Client, Error};
use crate::privacy::http::{http_request_over, with_default_json_accept};
...
impl Client {
    ...
    /// never returned).
    ///
    /// See [`Client::http_get`] for a description of the retry arguments and the
    /// timeouts applied to each attempt.
    ...
    /// setting them in the closure. See [`Client::http_get`] for the [`Builder`] methods
    /// you must not call within it.
    ///
    /// See [`Client::http_get`] for a description of the retry arguments and the
    /// timeouts applied to each attempt.
