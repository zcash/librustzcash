...
use super::{
    ...
    verify_spend_sig_internal, Node, Note, Nullifier, PaymentAddress, ProofGenerationKey, Rseed,
};
...
pub fn sign_message<Pr: SpendProver>(
    ...
    let (nullifier, zkproof) = create_address_proof(
...
pub(crate) fn verify_address_proof(
    ...
    // Parse the proof.
    let zkproof = Proof::read(&zkproof[..]).map_err(|_| ())?;
