...
mod tests {
    ...
    use zcash_note_encryption::{
        batch, NoteEncryption, OutgoingCipherKey, ENC_CIPHERTEXT_SIZE, NOTE_PLAINTEXT_SIZE,
    ...
    fn test_vectors() {
        ...
        for tv in test_vectors {
            ...
            {
                [Some(((decrypted_note, decrypted_to, decrypted_memo), _i))] => {
                    assert_eq!(decrypted_note, &note);
                    assert_eq!(decrypted_to, &to);
                    assert_eq!(&decrypted_memo.as_array()[..], &tv.memo[..]);
                }
            ...
            {
                [Some(((decrypted_note, decrypted_to), _i))] => {
                    assert_eq!(decrypted_note, &note);
                    assert_eq!(decrypted_to, &to);
                }
                _ => panic!("Note decryption failed"),
