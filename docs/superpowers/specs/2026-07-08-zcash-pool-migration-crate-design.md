...
| D3 | **No serde anywhere in the crate** | Upstream policy forbids derived serde outside marked cases; the only argument for it (prototype FFI wire-format compat) is void because the SDK FFI is being rewritten. The FFI defines its own DTOs via this crate's accessors/constructors. Adding a `serde` feature later would be non-breaking if ever needed. |
