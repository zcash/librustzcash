...
impl UnifiedAddressRequest {
    ...
    pub fn new(
        ...
        let has_metadata = expiry_height.is_some() && expiry_time.is_some();
        if !(has_shielded || (has_p2pkh && has_metadata)) {
            None
        } else {
            Some(Self {
                has_orchard,
...
impl UnifiedIncomingViewingKey {
    ...
    pub fn find_address(
        ...
        loop {
            let res = self.address(_j, request);
            match res {
                Ok(ua) => {
                    return Ok((ua, _j));
                }
