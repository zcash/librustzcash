...
impl UnifiedFullViewingKey {
    ...
    pub fn decode<P: consensus::Parameters>(params: &P, encoding: &str) -> Result<Self, String> {
        let (net, _revision, ufvk) =
            unified::Ufvk::decode(encoding).map_err(|e| e.to_string())?;
    ...
    fn to_ufvk(&self) -> unified::Ufvk {
        ...
        let data_items =
            core::iter::empty().chain(self.unknown.iter().map(|(typecode, data)| {
                unified::Fvk::Unknown {
                    typecode: *typecode,
                    data: data.clone(),
                }
...
impl UnifiedIncomingViewingKey {
    ...
    /// [ZIP 316]: https://zips.z.cash/zip-0316
    pub fn decode<P: consensus::Parameters>(params: &P, encoding: &str) -> Result<Self, String> {
        let (net, _revision, uivk) =
            unified::Uivk::decode(encoding).map_err(|e| e.to_string())?;
        let expected_net = params.network_type();
        if net != expected_net {
            return Err(format!(
...
mod tests {
    ...
    fn uivk_round_trip() {
        ...
        let decoded =
            UnifiedIncomingViewingKey::parse(&unified::Uivk::decode(&encoded).unwrap().2)
                .unwrap();
        ...
        let decoded_with_t =
            UnifiedIncomingViewingKey::parse(&unified::Uivk::decode(encoded_with_t).unwrap().2)
                .unwrap();
        #[cfg(feature = "transparent-inputs")]
        assert_eq!(
            decoded_with_t.transparent.map(|t| t.serialize()),
