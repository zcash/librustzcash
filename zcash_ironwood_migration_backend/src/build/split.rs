...
use zcash_primitives::transaction::fees::zip317::{FeeRule as Zip317FeeRule, GRACE_ACTIONS};
use zcash_protocol::consensus::{BlockHeight, Parameters};
use zcash_protocol::memo::MemoBytes;
use zcash_protocol::value::Zatoshis;

use super::{BuildError, build_config, finalize_pczt, marginal_fee_zatoshi, output_action_index};
...
/// Exact ZIP-317 fee (in zatoshi) for a note-split transaction. The migration's Orchard bundle (a
/// post-NU6.3 send-to-self) keeps cross-address transfers disabled, so each spend and each output
/// occupies its own action, pairing with a fabricated zero-valued counterpart: the action count is
/// `n_spends + n_outputs`, floored at the ZIP-317 grace count. This matches the transaction builder's
/// `orchard_action_count` for the same bundle.
fn split_fee(n_spends: usize, n_outputs: usize) -> u64 {
    let actions = (n_spends + n_outputs) as u64;
...
mod tests {
    ...
    fn arb_finalize_input() -> impl Strategy<Value = (usize, Vec<u64>, u64)> {
        (
            ...
            .prop_map(|(n_spends, outputs, extra)| {
                let planned: u64 = outputs.iter().sum();
                let selected_total = planned + split_fee(n_spends, outputs.len()) + extra;
    ...
    proptest! {
        ...
        fn finalize_conserves_value((n_spends, outputs, selected_total) in arb_finalize_input()) {
            let planned: u64 = outputs.iter().sum();
            let (fee, change) = finalize_split_outputs(n_spends, selected_total, &outputs).unwrap();
            prop_assert_eq!(planned + change.unwrap_or(0) + fee, selected_total);
            prop_assert!(fee >= split_fee(n_spends, outputs.len()));
    ...
    fn leftover_at_or_below_one_action_is_paid_into_fee() {
        ...
        let n_spends = 1;
        let fee_without_change = split_fee(n_spends, outputs.len());
        let extra_action_cost = split_fee(n_spends, outputs.len() + 1) - fee_without_change;
    ...
    fn exact_total_has_no_change() {
        ...
        let n_spends = 2;
        let total = 350 + split_fee(n_spends, outputs.len());
        assert_eq!(
            finalize_split_outputs(n_spends, total, &outputs).unwrap(),
            (split_fee(n_spends, outputs.len()), None)
        );
    }
    ...
    fn finalize_rejects_bad_inputs() {
        // No outputs to size.
        assert!(matches!(
            finalize_split_outputs(1, 1_000_000, &[]),
            Err(BuildError::Balance(_))
        ));
        // Selected total cannot even cover the fee.
        assert!(matches!(
            finalize_split_outputs(1, 1, &[100]),
            Err(BuildError::Balance(_))
        ));
        // Fee is affordable but the planned outputs are not.
        let fee = split_fee(1, 1);
        assert!(matches!(
            finalize_split_outputs(1, fee + 50, &[100]),
            Err(BuildError::Balance(_))
        ));
    }
    ...
    /// decompositions the `note_splitting` tests exercise.
    fn plan_for(strategy_index: usize, balance: u64, rng: &mut ChaCha8Rng) -> NoteSplitPlan {
        match strategy_index {
            0 => {
                let strategy = RandomizedOneTwoFive::recommended();
            ...
            _ => {
                let strategy = CanonicalPowerOfTen::zip_draft();
                strategy.plan(balance, 0, rng)
            }
        }
    }
    ...
    fn assert_builds_planned_split(
        ...
        let build_rng = ChaCha8Rng::seed_from_u64(42);
        ...
        let expected_fee = split_fee(1, outputs.len() + 1);
    ...
    proptest! {
        ...
        /// The builder turns the note-splitting strategies' plans into balanced split PCZTs across a
        /// range of balances (whole-ZEC and sub-ZEC) and all three strategies, on a post-NU6.3
        /// network (the Ironwood migration's environment). This is the plan-to-PCZT integration over
        /// the same decompositions the `note_splitting` tests cover.
        #[test]
        fn builds_planned_splits(
            ...
            let plan = plan_for(strategy_index, balance, &mut plan_rng);
            // A balance below one self-funding note has nothing to split; that case is covered by
            // `every_strategy_below_min_note_migrates_nothing` in `note_splitting`.
            prop_assume!(!plan.migration_outputs().is_empty());

            let params = regtest_network(true);
            assert_builds_planned_split(&fvk, &plan, &params);
        }
    }

    /// A concrete near-maximum split (Canonical power-of-ten of a large balance yields tens of
    /// cap-sized crossings): the whole many-output bundle builds into one balanced PCZT, post-NU6.3.
    #[test]
    fn builds_a_large_many_output_split() {
        let fvk = account();
        let balance = 4_000 * COIN;
        let mut plan_rng = ChaCha8Rng::seed_from_u64(0);
        let plan = plan_for(2, balance, &mut plan_rng); // CanonicalPowerOfTen
        assert!(
