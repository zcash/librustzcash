...
//! composition rule is abstracted behind the [`DenominationStrategy`] trait, and two implementations
//! (in [`strategies`]) are provided so the choice can be made (and reviewed) later by selecting a
//! strategy rather than by rewriting code:
//!
...
pub use strategies::{CanonicalPowerOfTen, RandomizedOneTwoFive};
...
/// The default largest denomination (in whole ZEC) [`RandomizedOneTwoFive`] gives a single note:
/// `2 * 10^4 = 20_000` ZEC, itself a `{1, 2, 5} * 10^k` value. Capping the top denomination keeps
/// even a whale's crossings within the shared denomination set, so no single crossing is a
/// near-unique fingerprint.
pub const MIGRATION_MAX_DENOMINATION_ZEC: u64 = 20_000;
...
impl NoteSplitPlan {
    ...
    // Part of the NoteSplitPlan API; not yet read by the engine, exercised by the crate's tests.
    #[allow(dead_code)]
