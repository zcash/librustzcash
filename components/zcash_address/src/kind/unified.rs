...
impl Typecode {
    ...
    pub fn preference_order(a: &Self, b: &Self) -> cmp::Ordering {
        ...
        match (a, b) {
            ...
            (Typecode::Metadata(a), Typecode::Metadata(b)) => {
            ...
            (Typecode::Data(a), Typecode::Data(b)) => match (a, b) {
                ...
                | (Sapling, Sapling)
                | (P2sh, P2sh)
                | (P2pkh, P2pkh) => cmp::Ordering::Equal,
...
pub enum ParseError {
    ...
    /// does not recognize.
    NotUnderstood(u32),
    /// A transparent receiver was found in a Revision 1 Unified Address.
    TransparentReceiverInR1Address,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ParseError::NotUnderstood(tc) => {
                write!(f, "MUST-understand metadata typecode 0x{:02X} not recognized", tc)
            }
            ParseError::TransparentReceiverInR1Address => {
                write!(f, "Transparent receivers are not permitted in Revision 1 Unified Addresses")
            }
            ParseError::NoDataItems => {
                write!(f, "Unified container has no data items")
            }
            ParseError::InvalidMetadataLength { typecode, expected, actual } => {
                write!(
                    f,
...
pub(crate) mod private {
    ...
    use super::{
        MetadataItem, ParseError, Typecode, Uitem,
        METADATA_TYPECODE_MIN, MUST_UNDERSTAND_METADATA_MIN, METADATA_TYPECODE_MAX,
        PADDING_LEN,
    };
    ...
    pub trait SealedContainer: super::Container + core::marker::Sized {
        ...
        fn network_hrp(network: &NetworkType, revision: Revision) -> &'static str {
            match (network, revision) {
                ...
                (NetworkType::Main, Revision::R1) => Self::MAINNET_R1,
                (NetworkType::Test, Revision::R1) => Self::TESTNET_R1,
                (NetworkType::Regtest, Revision::R1) => Self::REGTEST_R1,
            }
        }
        ...
        fn hrp_network(hrp: &str) -> Option<(NetworkType, Revision)> {
            ...
            } else if hrp == Self::MAINNET_R1 {
                Some((NetworkType::Main, Revision::R1))
            } else if hrp == Self::TESTNET_R1 {
                Some((NetworkType::Test, Revision::R1))
            } else if hrp == Self::REGTEST_R1 {
                Some((NetworkType::Regtest, Revision::R1))
            } else {
                None
            }
        }
            ...
            while cursor.position() < encoded.len().try_into().unwrap() {
                ...
                if (METADATA_TYPECODE_MIN..=METADATA_TYPECODE_MAX).contains(&tc_val) {
                    ...
                    if tc_val >= MUST_UNDERSTAND_METADATA_MIN {
                        ...
                        // SHOULD-understand (unknown) metadata: valid in both R0 and R1.
        ...
        fn try_from_items_internal(
            ...
            for item in &items {
                ...
                if let Uitem::Data(d) = item {
                    ...
                    let dt = d.typecode();
                    if dt == Typecode::P2SH
                        && prev_code == Some(u32::from(Typecode::P2PKH))
                    {
                        return Err(ParseError::BothP2phkAndP2sh);
                    }

                    // Check transparent receiver in R1 address
                    if revision == Revision::R1 && Self::IS_ADDRESS && dt.is_transparent() {
                        return Err(ParseError::TransparentReceiverInR1Address);
                    }

                    if !dt.is_transparent() {
                        only_transparent = false;
                        if matches!(dt, Typecode::Data(super::DataTypecode::Sapling)
            ...
            match revision {
                ...
                Revision::R1 => {
                    if Self::IS_ADDRESS {
                        // R1 UAs: Must contain at least one shielded receiver.
                        if !has_shielded {
                            return Err(ParseError::OnlyTransparent);
                        }
                    } else {
                        // R1 UVKs: Must contain at least one data item (transparent-only OK).
                        if !has_data_item {
        ...
        fn parse_internal<T: Into<Vec<u8>>>(
            ...
            Self::parse_items(hrp, buf, revision)
...
pub trait Encoding: private::SealedContainer {
    ...
    fn decode(s: &str) -> Result<(NetworkType, Revision, Self), ParseError> {
        if let Ok(parsed) = CheckedHrpstring::new::<Bech32mZip316>(s) {
            ...
            let (net, revision) = Self::hrp_network(hrp)
    ...
    fn encode(&self, network: &NetworkType) -> String {
        let hrp = Self::network_hrp(network, self.revision());
