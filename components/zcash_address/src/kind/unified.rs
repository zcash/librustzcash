...
pub enum DataTypecode {
    ...
    ///
    /// In a Revision 2 viewing key, a P2SH viewing key item: the encoding of a [BIP 388]
    /// wallet policy, whose descriptor template and key information vector together
    /// determine the redeem script a P2SH receiver is derived from. Revision 0 assigns
    /// this typecode no meaning in a viewing key.
    ///
    /// [ZIP 316]: https://zips.z.cash/zip-0316
...
impl<T: private::SealedItem> Uitem<T> {
    /// Compares items by the canonical encoding order of a unified container:
    /// ascending typecode value, with ties broken by the raw item data.
    pub(crate) fn encoding_order(a: &Self, b: &Self) -> cmp::Ordering {
        fn typecode_value<T: private::SealedItem>(item: &Uitem<T>) -> u32 {
            match item {
                Uitem::Data(d) => u32::from(d.typecode()),
                Uitem::Metadata(m) => u32::from(m.typecode()),
            }
        }

        typecode_value(a)
            .cmp(&typecode_value(b))
            .then_with(|| match (a, b) {
                (Uitem::Data(a), Uitem::Data(b)) => a.data().cmp(b.data()),
                (Uitem::Metadata(a), Uitem::Metadata(b)) => a.data().cmp(&b.data()),
                // The typecode ranges for data and metadata items are disjoint, so
                // items with equal typecodes are of the same kind.
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ParseError::NotUnderstood(tc) => {
                write!(
                    f,
                    "MUST-understand metadata typecode 0x{tc:02X} not recognized"
                )
            }
...
pub(crate) mod private {
    ...
    pub trait SealedItem: Clone {
        ...
        fn write_raw_encoding<W: Write>(&self, mut writer: W) -> corez::io::Result<()> {
            let data = self.data();
            CompactSize::write(
                &mut writer,
                <u32>::from(self.typecode())
                    .try_into()
                    .expect("a typecode fits in a usize"),
            )?;
    ...
    fn write_metadata_raw_encoding<W: Write>(
        item: &MetadataItem,
        mut writer: W,
    ) -> corez::io::Result<()> {
        ...
        CompactSize::write(
            &mut writer,
            tc_val.try_into().expect("a typecode fits in a usize"),
        )?;
        CompactSize::write(&mut writer, data.len())?;
        writer.write_all(&data)
    }
    ...
    pub trait SealedContainer: super::Container + core::marker::Sized {
        ...
        fn try_from_items_internal(
            ...
            // A container of any revision must contain at least one data item; one that
            // holds only metadata items is rejected.
            if !has_data_item {
                return Err(ParseError::NoDataItems);
            }

            // A Revision 0 container must also contain at least one shielded item. This
            // requirement is dropped for Revision 2, where the `zu`/`tu` HRP-content
            // rules apply instead; those are enforced in `parse_internal` for decoding,
            // and in `encode` for encoding.
            if revision == Revision::R0 && only_transparent {
        ...
        fn parse_internal<T: Into<Vec<u8>>>(
            ...
            if revision == Revision::R2 && Self::IS_ADDRESS {
                ...
                // A `zu` address must not contain any transparent receiver. ZIP 316
                // imposes no further constraint on its contents: a `zu` address whose
                // only data item is of an unrecognised type is well-formed. A `tu`
                // address has no additional constraints beyond those enforced by
                // `try_from_items_internal`.
                if !is_ti_hrp && has_transparent {
