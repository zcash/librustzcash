...
impl SealedContainer for Ufvk {
    ...
    const MAINNET_R1: &'static str = constants::mainnet::HRP_UNIFIED_FVK_R1;
    const TESTNET_R1: &'static str = constants::testnet::HRP_UNIFIED_FVK_R1;
    const REGTEST_R1: &'static str = constants::regtest::HRP_UNIFIED_FVK_R1;
...
mod tests {
    ...
    fn truncated() {
        ...
        assert_matches!(
            Ufvk::parse_internal(Ufvk::MAINNET, &truncated_after_sapling_typecode[..], Revision::R0),
            Err(ParseError::InvalidEncoding(_))
        );
    }
    ...
    #[test]
    fn r1_transparent_only_ufvk() {
        // R1 UVKs allow transparent-only.
        let items = vec![Uitem::Data(Fvk::P2pkh([1; 65]))];
        let ufvk = Ufvk::try_from_items(Revision::R1, items).unwrap();
        assert_eq!(ufvk.revision(), Revision::R1);

        // Round-trip.
        let encoded = ufvk.encode(&NetworkType::Main);
        assert!(encoded.starts_with("urview1"));
