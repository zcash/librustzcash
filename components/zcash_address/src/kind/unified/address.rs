...
pub mod testing {
    ...
    pub fn arb_transparent_typecode() -> impl Strategy<Value = DataTypecode> {
        select(vec![DataTypecode::P2pkh, DataTypecode::P2sh])
    }

    /// A strategy to generate an arbitrary shielded (Sapling, Orchard, or unknown) data
    /// typecode.
    pub fn arb_shielded_typecode() -> impl Strategy<Value = DataTypecode> {
        prop_oneof![
            Just(DataTypecode::Sapling),
            Just(DataTypecode::Orchard),
            ((<u32>::from(DataTypecode::Orchard) + 1)..0xC0u32).prop_map(DataTypecode::Unknown)
        ]
    }

    /// A strategy to generate an arbitrary valid set of data typecodes containing at
    /// most one of the P2SH and P2PKH transparent typecodes.
    pub fn arb_typecodes() -> impl Strategy<Value = BTreeSet<DataTypecode>> {
    ...
    /// Generates an arbitrary sequence of Unified address items containing receivers
    /// corresponding to the provided set of typecodes, in canonical encoding order. The
    /// receivers of this address are likely to not represent valid protocol receivers,
    /// and should only be used for testing parsing and/or encoding functions that do not
    /// concern themselves with the validity of the underlying receivers.
    pub fn arb_unified_address_for_typecodes(
        typecodes: BTreeSet<DataTypecode>,
    ) -> impl Strategy<Value = Vec<Uitem<Receiver>>> {
        typecodes
            .into_iter()
            .map(|tc| match tc {
    ...
    pub fn arb_known_shielded_typecode() -> impl Strategy<Value = DataTypecode> {
        select(vec![DataTypecode::Sapling, DataTypecode::Orchard])
    }
    ...
    pub fn arb_r2_shielded_address() -> impl Strategy<Value = Address> {
        (
            prop::collection::btree_set(arb_known_shielded_typecode(), 1..2),
            arb_metadata_items(),
        )
            .prop_flat_map(|(shielded_tcs, metadata)| {
                arb_unified_address_for_typecodes(shielded_tcs).prop_map(move |mut items| {
    ...
    pub fn arb_r2_transparent_including_address() -> impl Strategy<Value = Address> {
        (
            ...
            .prop_flat_map(|(shielded_tcs, transparent_tc, metadata)| {
                let typecodes: BTreeSet<DataTypecode> = shielded_tcs
                    .into_iter()
                    .chain(Some(transparent_tc))
                    .collect();
                arb_unified_address_for_typecodes(typecodes).prop_map(move |mut items| {
                    items.extend(metadata.clone());
                    Address {
                        revision: Revision::R2,
                        items,
...
mod tests {
    ...
    use crate::{
        kind::unified::{Container, Encoding, MetadataItem, Uitem, private::SealedContainer},
    ...
    fn address_receiver_queries() {
        ...
        let ua = Address {
            ...
            items: vec![
                Uitem::Data(Receiver::Orchard([0; 43])),
                Uitem::Data(Receiver::Sapling([1; 43])),
                Uitem::Data(Receiver::P2pkh([2; 20])),
            ],
        };
        ...
        let transparent_only = Address {
            revision: Revision::R0,
            items: vec![Uitem::Data(Receiver::P2pkh([0; 20]))],
        };
    ...
    #[test]
    fn r2_rejects_metadata_only_container() {
    ...
    #[test]
    fn r0_rejects_metadata_only_container() {
        // A container holding only metadata items has no data item whatever its
        // revision, which is distinct from holding only transparent items.
        // A metadata typecode below the MUST-understand range (which starts at
        // `0xE0`), so that a Revision 0 container may carry it without being
        // rejected as not understood.
        const SHOULD_UNDERSTAND_METADATA_TYPECODE: u32 = 0xC0;

        let items = vec![Uitem::Metadata(MetadataItem::Unknown {
            typecode: SHOULD_UNDERSTAND_METADATA_TYPECODE,
            data: vec![],
        })];
        assert_eq!(
            Address::try_from_items(Revision::R0, items),
            Err(ParseError::NoDataItems)
        );
    }

    #[test]
    fn zu_address_allows_unrecognised_only_receiver() {
        // ZIP 316 requires only that a `zu` address contain no transparent receiver;
        // one whose sole data item is of an unrecognised type is well-formed, and must
        // round-trip through the `zu` encoding the encoder selects for it.
        ...
        let encoded = ua.encode(&NetworkType::Main);
        assert!(
            encoded.starts_with("zu1"),
            "expected a zu address: {encoded}"
        );
