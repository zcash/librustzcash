...
//!
//! [`Decodable::Args`] carries the context a decoder needs but cannot recover from the byte
//! stream, such as the consensus branch ID required to parse a v4 transaction. It is `()` for
//! self-describing encodings. This mirrors `binrw::BinRead::Args`.
//!
...
//!
//! `Args` indexes the whole construction by a context, turning one retract into a family of them,
//! `Args -> Retract(B)`.
//!
...
//!    paper's `Syntax` does.
//!
//! 2. **One codec per type.** Rust coherence permits at most one `impl Decodable for T`, so the
//!    trait necessarily names *the* canonical codec of a type. A type with two encodings needs
//!    two types; see [`Unprefixed`], which exists only because `Vec<T>` already spends its single
//!    impl on the length-prefixed framing. Making the codec a first-class *value* rather than a
//!    trait on the payload lifts this restriction, at the cost of losing the ergonomics of
//!    `value.write(w)`. That trade is the reason `zcash_history::Version` is shaped the way it is.
...
/// A type that can be parsed from its canonical binary encoding.
pub trait Decodable: Sized {
    /// Context required to decode, which the byte stream does not itself carry.
    ///
    /// `()` when the encoding is self-describing.
    type Args<'a>;
...
pub trait Codec: Encodable + Decodable + for<'a> DecodableCtxFree<'a> {}

impl<T> Codec for T where T: Encodable + Decodable + for<'a> DecodableCtxFree<'a> {}

/// Helper that pins [`Decodable::Args`] to `()` for every lifetime.
///
/// A higher-ranked equality bound cannot be written directly on a generic associated type, so
/// this one-method trait stands in for `for<'a> Decodable<Args<'a> = ()>`.
pub trait DecodableCtxFree<'a>: Decodable<Args<'a> = ()> {}

impl<'a, T> DecodableCtxFree<'a> for T where T: Decodable<Args<'a> = ()> {}
