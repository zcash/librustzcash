...
mod tests {
    ...
    fn check_most_recent_boundary_golden(height: u32, expected: u32) {
        let b = most_recent_boundary(height);
    ...
    /// each: a multiple of the modulus, strictly above `act`, strictly below `most_recent`, at/after
    /// `funding`, and with an age in `[1, ANCHOR_AGE_CAP]`.
    fn check_anchor_golden(act: u32, funding: u32, most_recent: u32, seed: u64, expected: &[u32]) {
        let mut r = rng(seed);
        let got: Vec<u32> = (0..expected.len())
            .map(|_| draw_anchor_boundary(act, funding, most_recent, &mut r).unwrap())
            .collect();
    ...
    /// (`act = 144`, `funding = 288`, `most_recent = 2880`); each pinned sequence is the exact
    ...
    fn draw_anchor_boundary_golden() {
        ...
        check_anchor_golden(act, funding, most_recent, 1, &exp_seed1);
        check_anchor_golden(act, funding, most_recent, 42, &exp_seed42);
        check_anchor_golden(act, funding, most_recent, 7, &exp_seed7);
        check_anchor_golden(act, funding, most_recent, 100, &exp_seed100);
    }
    ...
    fn draw_anchor_boundary_bounded_golden() {
        fn check(seed: u64, expected: &[Option<u32>]) {
            ...
            for _ in 0..expected.len() {
                match draw_anchor_boundary_bounded(act, funding, most_recent, &counts, &mut r) {
                    Some(b) => {
                        counts.record(b);
                        got.push(Some(b));
                    }
    ...
    fn boundary_counts_golden() {
        ...
        assert_eq!(c.count(144), 0, "empty tally counts zero");
        ...
        assert_eq!(c.record(288), exp_record_288_first);
        assert_eq!(c.record(144), exp_record_144_first);
        assert_eq!(c.record(288), exp_record_288_second);
        assert_eq!(c.record(288), exp_record_288_third);
        assert_eq!(c.count(288), 3);
        assert_eq!(c.count(144), 1);
        assert_eq!(c.count(432), 0, "unrecorded boundary counts zero");
        // cohorts sorted ascending by boundary height, not insertion order.
        let exp_cohorts = alloc::vec![(144, 1), (288, 3)];
    ...
    proptest! {
        ...
        fn group_by_boundary_tallies(boundaries in prop::collection::vec(0u32..2000, 0..64)) {
            let counts = group_by_boundary(&boundaries);
            ...
            for &b in &boundaries {
                ...
                prop_assert_eq!(counts.count(b), occ);
            }
            // A boundary outside the drawn range is never present.
            prop_assert_eq!(counts.count(9999), 0);
    ...
    fn check_schedule_golden_pairs(
        ...
        let schedules = schedule(commit, n, &mut rng(seed));
        let broadcast: Vec<u32> = schedules.iter().map(|s| s.broadcast_height()).collect();
        let expiry: Vec<u32> = schedules.iter().map(|s| s.expiry_height()).collect();
        assert_eq!(
        ...
        assert_eq!(
            broadcast,
            schedule_broadcast_heights(commit, n, &mut rng(seed)),
        ...
        for (&b, &e) in broadcast.iter().zip(&expiry) {
            ...
            assert_eq!(e, expiry_height(b), "expiry {e} != expiry_height({b})");
        }
    }
    ...
    proptest! {
        ...
        fn schedule_pairs_broadcast_and_expiry(commit in 0u32..1_000_000,
            ...
            let broadcast: Vec<u32> = schedules.iter().map(|s| s.broadcast_height()).collect();
            prop_assert_eq!(&broadcast, &schedule_broadcast_heights(commit, n, &mut rng(seed)));
            let mut prev = commit;
            for s in &schedules {
