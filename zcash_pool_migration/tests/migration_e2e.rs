...
//!
//! These tests build a real `PostNu6_3` proving key and generate genuine Orchard/Ironwood proofs,
//! so the whole module is gated behind the workspace `expensive-tests` feature (matching the CI
//! "Slow Test" job, which runs `--features expensive-tests`) rather than running on every
//! `cargo test`.
...
fn note_split_plans_and_signs_against_a_seeded_wallet() {
    // 12.0007 ZEC decomposes into [10, 1] ZEC self-funding notes under the converged split-fee
    // estimate used by `prepare_note_split`; the residual stays as ordinary Orchard change.
    ...
    assert_eq!(
        orchard.actions().len(),
        1 + n_denoms + 1,
        "one action per spend (1), one per migration note, plus one plain Orchard residual-change output"
    );
    assert!(
...
/// second becomes due only after the chain advances past its scheduled send height.
#[test]
fn next_due_transfer_is_height_gated() {
    ...
    let first_due_height = u32::from(schedule.transfers()[0].next_executable_after_height());
    let second_due_height = u32::from(schedule.transfers()[1].next_executable_after_height());
    let blocks_to_advance = second_due_height
        .saturating_sub(first_due_height)
        .saturating_add(1);
    st.add_empty_blocks(blocks_to_advance as usize);

    let second = ctx
        .next_due_transfer()
        .unwrap()
        .expect("second transfer due after advancing to its scheduled send height");
    assert_ne!(
        first.txid(),
        second.txid(),
