...
impl<P: Parameters + Clone> MigrationContext<P> {
    ...
    /// How long an engine connection waits on a locked database before giving up with
    /// `SQLITE_BUSY`. The engine shares the wallet database file with the platform's own
    /// connections (notably the scanner's writer), so without a busy timeout a concurrent write
    /// would fail these calls immediately rather than briefly waiting for the lock to clear.
    ...
    pub fn sign_and_store_migration_schedule(
        ...
        let conn = self.store_conn()?;
        // Reuse an active run if one exists (e.g. a split already in progress). If none does, DO
        // NOT create one yet: a run inserted here would be stranded in `ReadyToMigrate` (which
        // `active_run` keeps returning) if signing then failed. Instead, sign under a provisional
        // id and only persist the new run once signing has succeeded, atomically with its pending
        // rows and phase — mirroring the sign-first-then-persist guarantee `sign_note_split` and
        // the external-signer `store_signed_schedule_pczts` both uphold.
        let existing_run = self.active_run(&conn)?.map(|r| r.run_id);
        let run_id = existing_run.clone().unwrap_or_else(new_run_id);
        let mut db = self.open_wallet()?;
        ...
        // `sign_schedule` has already inserted this run's pending rows (keyed by `run_id`) as it
        // went, so a brand-new run and its rows must be committed together: insert the run, then
        // set its scheduled phase, in one transaction. For a reused run only the phase changes.
        let tx = conn.unchecked_transaction()?;
...
mod tests {
    ...
    #[test]
    fn parse_txid_hex_is_the_inverse_of_txid_display() {
        // Round-trips against `TxId`'s own `Display` (byte-reversed hex, the block-explorer
        // convention), which is exactly the form the store persists as `txid_hex`.
        ...
        assert_eq!(parse_txid_hex(&hex).unwrap(), txid);

        // Malformed input is rejected, never silently defaulted.
        assert!(parse_txid_hex("").is_err()); // wrong length
        assert!(parse_txid_hex(&"a".repeat(63)).is_err()); // odd length
        assert!(parse_txid_hex(&"z".repeat(64)).is_err()); // non-hex
    }
