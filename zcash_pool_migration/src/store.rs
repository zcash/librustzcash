...
fn now_ms() -> i64 {
    // A clock before the Unix epoch is not expected on any platform this runs on; fall back to 0
    // rather than panicking, since these timestamps only order `active_run` selection and are not
...
/// Renders [`TERMINAL_PHASES`] as a quoted, comma-separated SQL `IN (...)` list.
///
/// This is the one place the crate interpolates values into SQL rather than binding them. It is
/// safe because `TERMINAL_PHASES` is a fixed set of hard-coded string constants (never user
/// input); any future edit MUST keep that invariant, or switch to bound parameters.
...
fn map_pending_row(row: &rusqlite::Row) -> rusqlite::Result<PendingTxRow> {
    // Named gets (not positional): the column order in `PENDING_COLUMNS` can be edited without
    // silently corrupting this mapping. Values/fees are stored as `i64` (SQLite has no unsigned
    // integer type) and read back as `u64`; real zatoshi amounts (max ~2.1e15) are far below
    // `i64::MAX`, so the round-trip is always exact.
    Ok(PendingTxRow {
        txid_hex: row.get("txid_hex")?,
        raw_pczt: row.get("raw_pczt")?,
        anchor_height: row.get("anchor_height")?,
        target_height: row.get("target_height")?,
        next_executable_after_height: row.get("next_executable_after_height")?,
        expiry_height: row.get("expiry_height")?,
        value_zatoshi: row.get::<_, i64>("value_zatoshi")? as u64,
        fee_zatoshi: row.get::<_, i64>("fee_zatoshi")? as u64,
        selected_note_txid: row.get("selected_note_txid")?,
        selected_note_output_index: row.get("selected_note_output_index")?,
        selected_note_value: row.get::<_, i64>("selected_note_value")? as u64,
