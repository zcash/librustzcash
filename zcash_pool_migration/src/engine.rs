...
/// does — which is why `ask` is a parameter here and of no other drive step. (The other place it
...
where
    ...
    R: RngCore + rand_core::CryptoRng,
{
    rebuild_expired_transfer_inner(params, backend, state, id, rng, Signing::InProcess(ask))
        .map(|_| ())
}
    ...
    R: RngCore + rand_core::CryptoRng,
{
    ...
    let (bytes, new_state, unsigned) = match signing {
        Signing::InProcess(ask) => {
            let signed = crate::build::sign_pczt(pczt, ask).map_err(RebuildError::Build)?;
...
#[derive(Clone, Copy)]
enum Signing<'a> {
    /// Sign each built PCZT here, with this Orchard spend-authorizing key.
    InProcess(&'a orchard::keys::SpendAuthorizingKey),
...
fn finish_built_pczt<E>(
    ...
    match signing {
        Signing::InProcess(ask) => {
            let signed = crate::build::sign_pczt(pczt, ask).map_err(CommitError::Build)?;
...
/// a specific policy.
///
/// `ask` is the account's Orchard spend authority, and every transaction of the run is signed with
/// it before this returns. It is an argument rather than something `backend` holds so that a
/// wallet need not keep the account's spending key reachable in order to plan, build or store a
/// migration: the key is live for this call. A caller holding the account's `UnifiedSpendingKey`
/// passes `SpendAuthorizingKey::from(usk.orchard())`; a caller that cannot (a watch-only account,
/// or one whose key is on a device) uses [`build_preparation_unsigned`], which takes no authority
/// and hands the unsigned transactions out to be signed elsewhere. The key must be the one whose
/// full viewing key `backend` reports, or the signatures authorize nothing the transactions spend.
#[cfg(feature = "orchard")]
...
/// policy; the other parameters are as [`commit_preparation`]'s, including `ask`, the account's
/// Orchard spend authority, with which every transaction of the run is signed before this returns.
...
mod commit_tests {
    ...
    fn rebuild_expired_transfer_reschedules_and_resigns_in_place() {
        ...
        rebuild_expired_transfer(&params, &backend, &backend.ask, &mut state, id, &mut rng)
    ...
    fn rebuild_rejects_a_replaced_funding_note_of_equal_value() {
        ...
        let mut rng = ChaCha8Rng::seed_from_u64(seed + 2);
        assert!(matches!(
            rebuild_expired_transfer(&params, &backend, &backend.ask, &mut state, tx.id, &mut rng),
    ...
    fn rebuild_expired_transfer_rejects_ineligible_transactions() {
        ...
        assert!(matches!(
            rebuild_expired_transfer(&params, &backend, &backend.ask, &mut state, unknown, &mut rng),
    ...
    fn a_changed_anchor_grid_is_rejected_rather_than_left_unprovable() {
        ...
        let mut rng = ChaCha8Rng::seed_from_u64(seed + 2);
        assert!(matches!(
            rebuild_expired_transfer(&params, &backend, &backend.ask, &mut state, transfer_id, &mut rng),
