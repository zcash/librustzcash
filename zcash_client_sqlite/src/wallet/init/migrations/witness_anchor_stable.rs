...
use super::{ironwood_received_notes, orchard_note_version, witness_stabilized_notes};
...
// `witness_stabilized_notes` adds the column this migration replaces; `ironwood_received_notes`
// creates the Ironwood table (with a `witness_stabilized` column) that this migration also
// converts. The `orchard_note_version` dependency pins the order in which columns are appended
// to `orchard_received_notes`, so that the final schema is deterministic.
const DEPENDENCIES: &[Uuid] = &[
    witness_stabilized_notes::MIGRATION_ID,
    orchard_note_version::MIGRATION_ID,
...
impl<P: consensus::Parameters> RusqliteMigration for Migration<P> {
    ...
    fn up(&self, transaction: &rusqlite::Transaction) -> Result<(), WalletMigrationError> {
        ...
        #[cfg(feature = "orchard")]
        transaction.execute_batch(&backfill(IRONWOOD_TABLES_PREFIX, IRONWOOD_SHARD_HEIGHT))?;
        // Without the `orchard` feature the orchard and ironwood tables exist but no
        // received notes can have been recorded in them, so the backfill is
        // unnecessary; we still drop the column below so the schema matches across
...
mod tests {
    ...
    fn migrate_backfills_anchor_heights() {
        ...
        #[cfg(feature = "orchard")]
        {
            let ironwood_pos_shard_0: i64 = 1;
            let ironwood_pos_shard_1: i64 = 1 << IRONWOOD_SHARD_HEIGHT;

            for (action_index, position, witness_stabilized) in [
                (0, ironwood_pos_shard_0, 1),
                (1, ironwood_pos_shard_0, 0),
                (2, ironwood_pos_shard_1, 0),
            ] {
                db_data
                    .conn
                    .execute(
                        "INSERT INTO ironwood_received_notes (
                             transaction_id, action_index, account_id,
                             diversifier, value, rho, rseed, is_change,
                             commitment_tree_position, note_version, witness_stabilized
                         ) VALUES (1, :action_index, 1, X'00', 0, X'00', X'00', 0, :position, 3, :stabilized)",
                        named_params![
                            ":action_index": action_index,
                            ":position": position,
                            ":stabilized": witness_stabilized,
