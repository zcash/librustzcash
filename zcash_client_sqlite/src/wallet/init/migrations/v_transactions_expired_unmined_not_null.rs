//! Makes `expired_unmined` a total boolean in `v_transactions` and
//! `v_transactions_with_pending_migrations`.
//!
//! The column compares a transaction's expiry height against the maximum height in `blocks`. A
//! wallet that has scanned no blocks has no such maximum, so the comparison yielded NULL and the
//! whole column with it, for every unmined transaction the wallet held. A consumer reading the
//! column as a boolean — which is what it is — failed to decode it.
//!
//! With no scanned blocks there is no height for a transaction to have expired against; that is
//! the not-expired case, not an unknown one. Both views now report `0` there.
...
use super::v_migration_transactions;
use crate::wallet::init::WalletMigrationError;

/// Identifies this migration in the wallet's schema-migration DAG.
pub const MIGRATION_ID: Uuid = Uuid::from_u128(0x3f1a7d62_5c48_4b19_9e73_8a04c6bd21f5);

pub(super) const DEPENDENCIES: &[Uuid] = &[v_migration_transactions::MIGRATION_ID];
...
impl schemerz::Migration<Uuid> for Migration {
    ...
    fn description(&self) -> &'static str {
        "Reports `expired_unmined` as false, not NULL, when no blocks have been scanned."
    }
}
...
impl RusqliteMigration for Migration {
    ...
    fn up(&self, transaction: &rusqlite::Transaction) -> Result<(), Self::Error> {
        // `v_transactions_with_pending_migrations` selects from `v_transactions`, so it is
        // dropped first and recreated last.
        transaction.execute_batch(&format!(
            "DROP VIEW v_transactions_with_pending_migrations;
             DROP VIEW v_transactions;
             {};
             {};",
            crate::wallet::db::VIEW_TRANSACTIONS,
...
mod tests {
    ...
    use super::{MIGRATION_ID, Migration};
    use crate::{
        WalletDb,
    ...
    /// The reverse migration restores both views. `test_migrate` exercises only `up`, and the
    /// `down` body is the one copy of the previous definitions that nothing else checks.
    #[test]
    fn migrate_down_restores_both_views() {
        ...
        // Both views must exist and be queryable; a view whose body failed to parse would have
        // been rejected at CREATE, and one referring to a dropped view fails on use.
        for view in [
            "v_transactions",
            "v_transactions_with_pending_migrations",
        ] {
            tx.query_row(&format!("SELECT COUNT(*) FROM {view}"), [], |row| {
                row.get::<_, i64>(0)
            })
            .unwrap_or_else(|e| panic!("{view} is not queryable after `down`: {e}"));
        }
    }
}
