...
//! that have a redeem script but no imported public key and no transparent child index.
...
impl RusqliteMigration for Migration {
    ...
    fn up(&self, transaction: &rusqlite::Transaction) -> Result<(), Self::Error> {
        ...
        //   as long as key_scope = -1 (Foreign) AND redeem_script IS NOT NULL
...
mod tests {
    ...
    fn migrate_preserves_data() {
        ...
        // Verify each row has redeem_script IS NULL.
        let rows: Vec<(String, Option<Vec<u8>>)> = {
            let mut stmt = db_data
                .conn
                .prepare(
                    "SELECT address, redeem_script FROM addresses
        ...
        db_data
            ...
            .execute(
                "INSERT INTO addresses (account_id, key_scope, address,
                 cached_transparent_receiver_address, receiver_flags, redeem_script)
                 VALUES (?1, -1, 't_p2sh', 't_p2sh', 2, X'0102030405')",
                [account_id],
            )
            .unwrap();

        // 3. Verify invalid P2SH is rejected (redeem_script NOT NULL but
        //    cached_transparent_receiver_address IS NULL).
        ...
        // 4. Verify non-foreign address with redeem_script is rejected.
        let result = db_data.conn.execute(
            "INSERT INTO addresses (account_id, key_scope, diversifier_index_be, address,
             cached_transparent_receiver_address, receiver_flags, redeem_script)
