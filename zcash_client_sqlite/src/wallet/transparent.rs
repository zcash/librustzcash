...
pub(crate) fn get_transparent_receivers<P: consensus::Parameters>(
    ...
    while let Some(row) = rows.next()? {
        ...
        if let Some(taddr) = taddr {
            ...
            let metadata = if let Some(ref rs_bytes) = redeem_script_bytes {
                ...
                if matches!(key_scope, KeyScope::Foreign) {
                    ...
                    return Err(SqliteClientError::CorruptedData(
                        "Redeem script found for non-foreign-scoped address, which is not supported."
                            .to_owned(),
                    ));
                }
            } else {
            ...
            #[cfg(not(feature = "transparent-key-import"))]
            let metadata = non_script_metadata()?;
            ...
            |row| {
                ...
                Ok(KeyScope::decode(scope_code).and_then(|key_scope| {
                    ...
                    match key_scope.as_transparent().zip(address_index) {
                        ...
                        None => {
                            let _redeem_script_bytes: Option<Vec<u8>> = row.get("redeem_script")?;
                            ...
                            #[cfg(feature = "transparent-key-import")]
                            {
                                if let Some(ref rs_bytes) = _redeem_script_bytes {
                                    use zcash_script::script::{self, Code};
                                    ...
                                    Err(SqliteClientError::CorruptedData(
                                        "imported_transparent_receiver_pubkey or redeem_script must be set for \"standalone\" transparent addresses".to_string()
                                    ))
                                }
                            }
                            ...
                            #[cfg(not(feature = "transparent-key-import"))]
                            {
                                if _pubkey_bytes.is_some() || _redeem_script_bytes.is_some() {
                                    Err(SqliteClientError::CorruptedData(
