...
///   `cached_transparent_receiver_address` is non-null, either this column, or `redeem_script`
///   (for imported P2SH addresses), or `transparent_child_index` must also be non-null.
/// - `redeem_script`: The serialized redeem script for an imported standalone P2SH address. When
///   present, `cached_transparent_receiver_address` holds the P2SH address derived from this
///   script. This may be set for imported addresses (key_scope = -1) or derived addresses, e.g. in
///   the case of a ZIP 48 multisig.
///
            ...
            OR (
                ...
                AND (
                    (transparent_child_index IS NULL) == (
                        ...
                        OR (key_scope = -1 AND redeem_script IS NOT NULL)
                    )
                )
            )
        )
