...
mod tests {
    ...
    /// Check 3 of the spendability rule: a note in a shard that is not scan-clean is durable
    /// only if its stored floor reaches the bottom of the pruning window (no unscanned region
    /// separates it), while a scan-clean shard is durable regardless.
    #[test]
    fn is_note_spendable_check3_pruning_region_gate() {
        ...
        // anchor available, confirmations met. Vary only (shard_scan_clean, gap_top, floor).
        let spendable = |shard_scan_clean, gap_top: Option<u32>, floor: u32| {
        ...
        // Shard not scan-clean, floor at or above the top of the gap: durable.
        assert!(spendable(false, Some(100), 100));
        assert!(spendable(false, Some(90), 100));
        // Shard not scan-clean, floor below the gap top: NOT durable — the hole check 3 closes.
        assert!(!spendable(false, Some(101), 100));
        // A scan-clean shard holds every leaf after the note, so the gap is irrelevant.
