...
use {
    ...
    zcash_client_backend::data_api::chain::BlockCache,
};
...
impl BlockCache for FsBlockDb {
    ...
    fn truncate(&self, block_height: BlockHeight) -> Result<(), Self::Error> {
        ...
        chain::blockmetadb_truncate_to_height(&self.conn.lock().unwrap(), block_height)?;

        Ok(())
    }

    /// Deletes a range of blocks from the block cache.
    fn delete(&self, range: &ScanRange) -> JoinHandle<Result<(), Self::Error>> {
        ...
        let block_meta: Vec<Result<Option<BlockMeta>, FsBlockDbError>> = (start..end)
            .map(|height| self.find_block(BlockHeight::from_u32(height)))
            .collect();

        tokio::spawn(async move {
            for meta_result in block_meta {
                if let Some(meta) = meta_result? {
                    tokio::fs::remove_file(meta.block_file_path(block_cache_root.as_ref()))
                        .await
                        .map_err(FsBlockDbError::Fs)?
                }
            }
...
mod tests {
    ...
    use zcash_client_backend::data_api::{
        chain::BlockCache, AccountBirthday, WalletRead, WalletWrite,
    };
    ...
    pub(crate) fn fsblockdb_api() {
        ...
        let mut st = TestBuilder::new()
            .with_fs_block_cache(&fsblockdb_root)
            .build();
        ...
        assert_eq!(
            st.cache().find_block(h2).unwrap().map(|meta| meta.height),
            Some(h2)
        );
