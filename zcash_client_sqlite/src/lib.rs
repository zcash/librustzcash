...
mod tests {
    ...
    fn find_account_for_address_returns_none_for_unknown_address() {
        ...
        assert_eq!(
            st.wallet().find_account_for_address(&unknown_address).unwrap(),
            None
        );
    }
    ...
    fn find_account_for_ua_finds_via_transparent_receiver_cache() {
        ...
        let t1 = UnifiedSpendingKey::from_seed(&state.network(), &[7u8; 32], zip32::AccountId::ZERO)
            .expect("valid seed")
    ...
    fn find_account_for_ua_errors_when_receivers_map_to_different_accounts() {
        ...
        let invalid_address = Address::Unified(
            UnifiedAddress::from_receivers(Some(orchard_receiver_2), Some(sapling_receiver_1), None)
    ...
    fn generate_unified_address_with_all_available_keys(state: &mut TestState<(), TestDb, LocalNetwork>, account_id: AccountUuid) -> (UnifiedAddress, DiversifierIndex) {
        state
            .wallet_mut()
    ...
    fn remove_account_from_db(state: &mut TestState<(), TestDb, LocalNetwork>, acc1_id: AccountUuid) -> i64 {
