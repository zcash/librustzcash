...
- The spendability rule for shielded notes now uses a per-note
  ...
  *floor* on the anchors that can witness the note: the wallet has the data
  ...
  window; the note's witness region below that window is durable (every block
  in its shard's range has been scanned, or its floor reaches the bottom of the
  window with no unscanned range in between); the chosen anchor's tree root is
  constructable; and the note has met its confirmations-policy threshold.
  Migration to this schema is
  automatic.
