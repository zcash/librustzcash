...
- The spendability rule for shielded notes now uses a per-note
  ...
  *floor* on the anchors that can witness the note: the wallet has the data
  ...
  witness region below the window is durable (every block in its shard's
  range has been scanned, or its floor reaches the bottom of the window with
  no unscanned range in between);
