...
use mmr::{Tree as MMRTree, Entry as MMREntry, NodeData as MMRNodeData};
    ...
    for i in 0..p_len {
        peaks.push((
            ...
            match MMREntry::from_bytes(cbranch,&nodes[i][..]) {
                Ok(entry) => entry,
                _ => { return Err("Invalid encoding"); } // error
            }
        ));
    }
...
pub extern "system" fn librustzcash_mmr_append(
    ...
    let new_node_bytes= unsafe {
        slice::from_raw_parts(nn_ptr, mmr::MAX_NODE_DATA_SIZE)
    };
    ...
    let root_node = tree.root_node().expect("Just added, should resolve always; qed");
    unsafe {
        slice::from_raw_parts_mut(rt_ret, 32)
            .copy_from_slice(&root_node.data().subtree_commitment);

        for (idx, next_buf) in
            slice::from_raw_parts_mut(buf_ret, return_count as usize)
                .iter_mut()
                .enumerate()
        {
